...
def classificar_entorno(
    ...
    *,
    ...
    limiares = {c: float(out[c].dropna().quantile(quantil)) for c in dimensoes}
    flags = pd.DataFrame(index=out.index)
    for c in dimensoes:
        # Ausência de observação não é carência: permanece NA e é excluída da contagem válida.
        flags[c] = out[c].ge(limiares[c]).where(out[c].notna())
    ...
    return out, ResultadoEntorno(limiares_p80=limiares, n_setores=len(out))
